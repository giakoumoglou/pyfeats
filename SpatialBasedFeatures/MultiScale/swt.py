...
import pywt
import numpy as np
from ..utilities import _pad_image

def swt_features(f, mask, wavelet='bior3.3',levels=3):
        
    # Step 1: Pad image so SWT can work
    f2 = _pad_image(f)       # pad to the next power of 2 in each dimension
    mask2 = _pad_image(mask) # pad to the next power of 2 in each dimension
